...
NSString* const  NetWebServiceRequestErrorDomain2 = @"NetWebServiceRequestErrorDomain2";
@implementation VideoWebRequest
...
-(void)requestPostWithAction:(NSString *)action WithParameter:(NSDictionary *)parameterDic successBlock:(SuccessResponeBlock)successBlock failedBlock:(FailedResponeBlock)failedBlock
{
    ...
    //如果url有中文，需要url编码
    NSString * strURL = [NSString stringWithFormat:@"%@%@",VIDEO_SERVER_IP,action];
...
//            [_delegate finishedWebRequest:self resultDic:dic];
            if([[dic objectForKey:@"success"] intValue] == 1){
                ...
                error = ERROR_INFO(NetWebServiceRequestErrorDomain2, statusCode,msg);
        ...
        else
        {
            ...
            error = ERROR_INFO(NetWebServiceRequestErrorDomain2, statusCode,@"serverData.error");
            [self requestError:error];
        }
    ...
    else
    {
        ...
        {
            error = ERROR_INFO(NetWebServiceRequestErrorDomain2, statusCode,request.responseStatusMessage);
        }
        //客户端有错误
        else if(statusCode >= 400 && statusCode < 500)
        {
            ...
            if(statusCode == 403)
            {
                error = ERROR_INFO(NetWebServiceRequestErrorDomain2, statusCode,@"无权限");
            }
            else if(statusCode == 404)
            {
                error = ERROR_INFO(NetWebServiceRequestErrorDomain2, statusCode,@"资源不存在");
            }
        }
        //服务器异常
        else if(statusCode >= 500 && statusCode < 600)
        {
            error = ERROR_INFO(NetWebServiceRequestErrorDomain2, statusCode,@"server.error");
        }
        [self requestError:error];
    }
...
//    [UIApplication sharedApplication].networkActivityIndicatorVisible = NO;
    ...
    NSError *error = ERROR_DICTINFO(NetWebServiceRequestErrorDomain2, request.error.code, request.error.userInfo);
 
    if (error.code == ASIRequestTimedOutErrorType)
    {
        error = ERROR_INFO(NetWebServiceRequestErrorDomain2, NetRequestTimedOutErrorType, @"net.timeout");
    }
    else if (error.code == ASIConnectionFailureErrorType)
    {
        error = ERROR_INFO(NetWebServiceRequestErrorDomain2, NetRequestConnectionFailureErrorType, @"requestFailed");
    }
    else if(error.code == ASIAuthenticationErrorType)
    {
        error = ERROR_INFO(NetWebServiceRequestErrorDomain2, NetRequestConnectionFailureErrorType, @"auth.failed");
    }
    else
    {
        error = ERROR_INFO(NetWebServiceRequestErrorDomain2, NetRequestConnectionFailureErrorType, @"requestFailed");
