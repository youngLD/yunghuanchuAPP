...
-(void)vedioPlay:(NSString *)videoName{
    NSArray *array = [videoName componentsSeparatedByString:@"."];
//    NSString* path = [[NSBundle mainBundle] pathForResource:[array firstObject] ofType:@"html"];
//    if(!path)
//    {
//        return;
//    }
...
//    [self.playerView.playerV loadRequest:request];
    NSURL *sourceMovieURL = [[NSBundle mainBundle] URLForResource:[array firstObject] withExtension:@"mp4"];
...
//        NSArray *resultAnswerAry = [self.questionItem.answerId componentsSeparatedByString:@";"];
        NSMutableArray *resultAnswerAry=[[NSMutableArray alloc]init];
        if (self.questionItem.subType != 1) {
             resultAnswerAry = [NSMutableArray arrayWithArray:@[self.questionItem.optAStr,self.questionItem.optBStr,self.questionItem.optCStr,self.questionItem.optDStr]];
        }
        ...
        //判断题
        if(self.questionItem.subType == 1){
            if([self.questionItem.answer isEqualToString:item.uid]){
                isItemTheCurrect = YES;
            }
        }
        else{
            
                if([self.questionItem.answer rangeOfString:item.uid].location !=NSNotFound){
                    isItemTheCurrect = YES;
                }
        }
    ...
    if(self.questionItem.isSelected){
        ...
        //多选
        if(self.questionItem.subType == 3){
            if(item.isSelected){
...
-(void)cellDidClick:(UIButton *)sender{
    //如果是非多选，并且还未选择
    if(self.questionItem.subType != 3 && !self.questionItem.isSelected){
    ...
    //多选
    else if(self.questionItem.subType == 3){
        if(!self.questionItem.isSelected){
            QuestionAnswerItem *item = self.dataAry[sender.tag];
            item.isSelected = !item.isSelected;
...
//            NSArray *resultAnswerAry = [self.questionItem.answer componentsSeparatedByString:@";"];
            if(self.questionItem.answer.length == selectCount){
                //默认为正确的
                BOOL currect = YES;
                
                for(int i=0;i<self.questionItem.answer.length;i++){
                    //正确答案中的一项
